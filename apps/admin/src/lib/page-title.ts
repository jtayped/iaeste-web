...
/**
 * One breadcrumb entry, root-first and *without* the `dashboard` root — the
 * shell prepends that. The last entry carries no `href` and renders as the
 * current page. A dynamic leaf (a person's name, a campaign label) is just a
 * `label` built from the record the page already loaded.
 */
...
export const TITLE_ROOT = "dashboard";

const SEPARATOR = " · ";

/**
 * Builds the full document title: `dashboard · <trail>`.
 *
 * A trail that already starts with `dashboard` (the dashboard page, whose title
 * *is* the root) is not prefixed twice — `adminTitle("dashboard")` is
 ...
 * to `<PageShell>`.
 *
 * `absolute` on purpose: the root layout sets a `dashboard · %s` template for
 * any route that forgets to export metadata, and `adminTitle` already applies
 * that prefix itself. Letting the template run over it would produce
 * `dashboard · dashboard · membres`.
 */
