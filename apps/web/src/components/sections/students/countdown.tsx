...
const Countdown = ({ target, label }: { target: string; label: string }) => {
  ...
  return (
    ...
    <div role="timer" aria-label={label}>
      <p className="text-xs tracking-[0.12em] text-primary-foreground/60 uppercase">
        {label}
      </p>
      <ul className="mt-3 flex flex-wrap gap-2.5">
        {UNITS.map((unit) => {
          ...
          return (
            <li
              key={unit}
              className="min-w-[4.75rem] rounded-xl border border-white/15 bg-white/5 px-3 py-2.5 text-center"
            >
